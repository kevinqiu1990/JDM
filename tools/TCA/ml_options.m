...
%             'lambda_A':  RKHS norm regularization parameter (Ambient)        :  1 
%             'lambda_I':  Manifold regularization parameter  (Intrinsic)      :  1    
% -------------------------------------------------------------------------------------
%
...
switch field
 ...
 case 'GraphNormalize'
   ...
   case 'ClassEdges'
     requireInt = 1;
     range=[0 1];  
  case {'NNType','Kernel', 'GraphWeights', 'GraphDistanceFunction'}
    if ~isString,
      valid = 0;
      errmsg = sprintf('Invalid value for %s parameter: Must be a string', field);
    end
  case {'lambda_A', 'lambda_I','GraphWeightParam'}
    range = [0 Inf];
  case 'KernelParam'
    valid = 1;
  case 'Degree'
